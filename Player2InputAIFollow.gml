	...
	if(!MainPlayer.Death) {
		...
		if(AngleCheck == 0) {
			if(InputLeft == true) { // Are we pressing left?
				...
				if(PosX < MainPlayerPosXOffestL and Xsp < 0){
					InputLeft = false;
				}
			}
			...
			if(InputRight == true) { // Are we pressing Right?
				...
				if(PosX > MainPlayerPosXOffestR and Ysp < 0) {
					InputRight = false;
				}
			}
		}
		...
		if(CheckGrav == true and MainPlayer.Xsp < 2 and MainPlayer.Xsp > -2)
		{
			targetMainPlayerPosX += PosOffset;	
			if(MainPlayer.Facing == FlipRight) { //Are They Facing RIght???????⁉️??????????
		...
		if(Animation == AnimPush) { // Are we Pushing?	
			// Start a timer
			autoJumpTimer++
		...
		if (GroundLock > 16 and (Xsp < 8 or Xsp > -8)) {
